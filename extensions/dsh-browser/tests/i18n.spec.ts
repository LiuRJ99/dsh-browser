...
describe('browser locale selection', () => {
  ...
  it('provides localized panel and prompt copy in both languages', () => {
    ...
    expect(PANEL_COPY.en.approval.trustOrigin).toBe('Add to trusted list')
    expect(PANEL_COPY.en.tabHandoff.keepAlways).toBe('Stay & stop asking')
    expect(PANEL_COPY.en.tabHandoff.askAgain).toBe('Ask on tab switch')
    ...
    expect(PANEL_COPY.zh.tabHandoff.keepAlways).toBe('留在原页面并不再询问')
    expect(PANEL_COPY.zh.tabHandoff.askAgain).toBe('切换标签页时重新询问')
  })
})
