...
export interface TabManagementContext {
  /** Whether the caller has explicitly opted into unrestricted browser access. */
  ...
  commitAction?: () => void
  /** Restore withdrawability when an action was not delivered. */
...
function contentScriptReceiverMissing(error: unknown): boolean {
  ...
  return /Receiving end does not exist|Could not establish connection|no receiver/i.test(error.message)
}
...
async function dispatchTabManagementTool(
  ...
  const rejected = await authorizeTabManagement(tabManagementApproval(call, before), authorize, call, signal)
  if (rejected !== undefined) return rejected
  ...
  if (call.name === 'browser_close_tab') {
    ...
    return {
      ok: true,
      result: { text: tabId === context.controlledTabId
 ...
 * @param tabManagement - tab collection access and affinity callbacks.
...
export async function dispatchToolCall(
  ...
  if (!isInjectablePage(tab.url)) {
    ...
    return await dispatchTabNativeTool(
      tab.id,
      tab.url,
      tab.title,
      tab.windowId,
    ...
    ) ?? unavailable('The current page is protected by the browser. Only browser-level navigation controls are available.')
  }
  try {
...
/** Parse a model-supplied http(s) URL for browser-level navigation. */
...
/**
 * Open an HTTP(S) URL in a new tab and let the caller commit session affinity.
 * The readiness listener is armed before navigation so fast pages cannot race it.
 */
export async function dispatchOpenTab(
  ...
  try {
    ...
    navigationWait.cancel()
    await removeCreatedTab(tabId)
    return { ok: false, error: { code: 'action-failed', message: error instanceof Error ? error.message : 'Failed to navigate the new browser tab.' } }
  }
  const ready = await navigationWait.ready
  if (isCancelled(call, signal)) {
    await removeCreatedTab(tabId)
  ...
  if (!bindCreatedTab(created)) {
    await removeCreatedTab(tabId)
    return { ok: false, error: { code: 'action-failed', message: 'The new tab could not become the controlled browser target.' } }
  }
  resetTabSnapshot(tabId)
  ...
  return { ok: true, result: { text: `${status} Call browser_snapshot again after the page loads.` } }
}
