...
- **本地自动化**：可信的本地调用方使用 host 的 `/ext/browser-control` 接口；扩展仍然是 `/ext/bridge` 的唯一 WebSocket 客户端，并在同一个受控标签页中执行请求。
...
- **标签页绑定**：提交提示时会在模型开始工作前绑定活动标签页；如果直接调用浏览器工具，也会在需要时完成首次绑定。默认情况下，手动切换标签页或窗口后，后续工具会暂停，并询问助手继续原页面还是跟随当前页。选择「留在原页面并不再询问」会固定原页面：这个针对具体绑定的显式选择优先于全局「自动跟随当前标签页」设置，因此之后切换标签页会在后台继续操作而不再询问，直到选择「切换标签页时重新询问」或发生其他绑定变更。也可在设置中开启「自动跟随当前标签页」；开启后不会强制切换你正在看的页面，但会切换助手的受控标签页、重置页面引用、撤销尚未完成的操作审批，并在受控页关闭且仍有当前页可用时自动恢复。没有可用活动标签页时，仍会保持失败关闭，直到有页面可供恢复。
- **分级审批**：默认「自动共享」允许模型按需读取受控标签页而不额外弹窗；「每次询问」可恢复逐次读取确认，「关闭」会阻断读取。`browser_list_tabs` 对所有标签页元数据也遵循这套策略：自动共享不弹窗、每次询问会弹窗、关闭则阻断。普通状态变更工具（包括本地文件上传和 JavaScript 执行）统一沿用 origin 信任；用户可仅允许一次、信任当前会话中的单个 origin，或将稳定 origin 加入永久可信名单。历史前进/后退因目标未知使用不写入永久设置的会话级权限；绑定/跟随标签页沿用目标标签页的 origin 信任；`browser_close_tab` 始终需要再次确认。会话级历史权限会在侧栏租约、桥接代际或会话结束时清除。最后一个侧栏关闭或 Service Worker 重启会清空临时信任；永久名单也可在设置中查看和移除，并适用于 JavaScript 执行和向受信任目标 origin 上传文件；本地路径仍会隐藏。侧栏关闭时，审批最多保留 60 秒；启用通知后，系统通知可把用户带回侧栏。会话级审批只会在其所属会话恢复完成后显示。调用方取消或桥接超时时，会先撤销尚未完成的审批，过期动作不会继续执行。
- **会话续接**：重新打开侧栏时默认恢复最近活跃的浏览器会话；若该会话不可用，则恢复最新的非空持久会话，最后才创建新会话。可在设置中关闭。

## 权限说明

Chrome 使用 `sidePanel`，Firefox 使用 `sidebar_action`。两者都申请 `storage`（设置与最近会话续接）、`notifications`（侧栏关闭时可选的审批提醒）、`tabs` + `activeTab` + `scripting`（观察切页，并向用户显式选择的受控标签页注入/发消息；安装前已打开的页面也会按需补注入）、`webNavigation`（枚举该标签页中的 frame，并把消息绑定到具体文档）、`alarms`（后台保活）和 `http/https`（内容脚本注入普通网页）。Firefox AMO manifest 如实声明扩展会把浏览活动、网页内容/操作和对话内容发送给用户配置的 dsh/模型服务。扩展绝不强制切换用户正在看的标签页；默认也不会静默跟随手动切页，但开启「自动跟随当前标签页」后会在不改变可见页面的前提下切换助手的受控目标。默认策略下，只有用户选择继续原页面后，助手才会在后台操作。

## 已知限制

- 同时只有一个扩展连接桥。未打开侧栏的浏览器 Profile 不会抢占连接；另一个已打开的侧栏顶替连接后，被替换的一端会主动让权，不再反复重连互踢。
- 每个专用对话会话都使用相互隔离的标签页，导航和状态彼此独立。
