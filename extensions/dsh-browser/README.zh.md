...
- **标签页绑定**：提交提示时会在模型开始工作前绑定活动标签页；如果直接调用浏览器工具，也会在需要时完成首次绑定。默认情况下，手动切换标签页或窗口后，后续工具会暂停，并询问助手继续原页面还是跟随当前页。可在设置中开启「自动跟随当前标签页」；开启后不会强制切换你正在看的页面，但会切换助手的受控标签页、重置页面引用、撤销尚未完成的操作审批，并在受控页关闭且仍有当前页可用时自动恢复。没有可用活动标签页时，仍会保持失败关闭，直到有页面可供恢复。
