...
- **Tab affinity**: prompt submission binds the active tab before the model starts working; a direct browser-tool call also performs the initial bind when needed. By default, a manual tab/window switch pauses later tools and asks whether the assistant should stay on the original tab or follow the newly visible one. Choosing **Stay & stop asking** pins the original tab: this explicit per-binding choice takes precedence over the global `autoFollowActiveTab` preference, so later switches stay in the background without prompting until **Ask on tab switch** or another binding change clears the pin. Settings can also opt into automatically following the active tab; this never changes the visible tab itself, but it does reset page-reference state, withdraw any open action approval, and recover to the active tab when the controlled tab closes. If no active tab remains, the session still fails closed until a page is available for recovery.
- **Proportional approval**: the default `auto` mode lets the model read the controlled tab without an extra prompt; `ask` restores per-read confirmation and `off` blocks reads. `browser_list_tabs` follows the same policy for all-tab metadata: it is silent in `auto`, asks in `ask`, and is blocked in `off`. State-changing tools, including local file uploads and JavaScript execution, use the same origin trust path and show their exact origin plus a redacted action summary. The user may deny, allow once, trust one origin for the current session, or add one stable origin to the permanent trusted list directly from the approval dialog. History navigation remains session-only because its destination is unknown; tab attach/follow use the target tab's origin trust; `browser_close_tab` always asks. Permanent origin trust also covers JavaScript execution and uploads to a trusted destination origin. Local file paths remain redacted. If the panel is closed, an approval remains pending for up to 60 seconds and, when enabled, a system notification opens the panel for review. The panel restores the requesting session before showing a session-scoped approval. Caller cancellation or bridge timeout withdraws any open approval before an action can run.
...
- Dedicated chat sessions keep isolated tabs with independent navigation and state.
