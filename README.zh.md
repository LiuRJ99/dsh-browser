...
> 本仓库为 [`Lum1104/dsh-browser`](https://github.com/Lum1104/dsh-browser) 的维护与增强分支（仓库：[`LiuRJ99/dsh-browser`](https://github.com/LiuRJ99/dsh-browser)，当前版本 `v0.1.7`）。在完整保留官方文本化控制与低延迟优势的基础上，针对多 Agent 协作、内存治理、多标签页并发与权限治理进行了重点增强。

### 1. 本 Fork 安装方式

使用包含最新 0.1.7 修复的安装脚本：
...
- 助手开始工作时会绑定当时的活动标签页（提交提示时绑定；直接调用浏览器工具时则在首次调用绑定）。默认情况下，用户手动切页后，后续浏览器操作会暂停，侧栏会询问让助手继续原页面还是跟随新页面；可在设置中开启「自动跟随当前标签页」，让助手在不切换用户可见页面的前提下自动改绑、重置页面引用，并在受控标签页关闭且仍有当前页可用时自动恢复。默认策略下，选择继续原页面后才允许助手在后台操作；若没有可用的活动标签页，工具仍会暂停。
...
- 点击、输入、文件上传、按键、普通导航、JavaScript 执行和刷新统一使用 origin 信任逻辑。历史前进/后退因目标未知使用当前会话级权限；绑定/跟随标签页沿用目标标签页的 origin 信任；`browser_close_tab` 始终重新询问。`browser_list_tabs` 遵循页面共享策略：自动共享不弹窗、每次询问弹窗、关闭时阻断。普通 origin 可在当前侧栏会话中临时信任，或在审批框/设置中加入永久可信名单；会话级历史权限不写入永久设置，并在侧栏租约、桥接代际或会话结束时清除。文件上传沿用目标 origin 的信任策略，但审批摘要仍会隐藏本地路径。显式跨域 `browser_navigate` 仍要求所有已知来源可信，未知历史目标使用会话级权限。
