...
/** Loopback HTTP control route for trusted local automation clients. */
const BRIDGE_CONTROL_PATH = "/ext/browser-control";
...
const BRIDGE_SESSION_PURGE_METHOD = "bridge.session.purge";
...
function parseBridgeFrame(text) {
	...
	switch (frame.t) {
		...
		case "tool.call":
			if (frame.sessionId !== void 0 && (typeof frame.sessionId !== "string" || frame.sessionId.trim() === "")) return void 0;
			return typeof frame.id === "string" && typeof frame.name === "string" && typeof frame.args === "object" && frame.args !== null && !Array.isArray(frame.args) && typeof frame.expiresAt === "number" && Number.isFinite(frame.expiresAt) && frame.expiresAt > 0 ? {
				t: "tool.call",
				id: frame.id,
				name: frame.name,
				args: frame.args,
...
//#endregion
export { BRIDGE_CONFIG_PATH, BRIDGE_CONTROL_PATH, BRIDGE_INJECT_BROWSER_SNAPSHOT_METHOD, BRIDGE_PATH, BRIDGE_SESSION_PURGE_METHOD, DEFAULT_SNAPSHOT_MAX_CHARS, DEFAULT_TOKEN_BYTES, HELLO_TIMEOUT_MS, MIN_SNAPSHOT_MAX_CHARS, PING_INTERVAL_MS, isClientFrame, isRespondResult, isServerFrame, parseBridgeFrame };
